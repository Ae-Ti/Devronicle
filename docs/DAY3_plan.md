...
GITHUB_REDIRECT_URI=http://192.168.0.10:5173/api/auth/github/callback   # 5173 프록시 경유 — 8080 을 열 필요가 없다
# GitHub OAuth App 의 Authorization callback URL 도 같은 값으로 (§8-1)
```

### 테스트

- `OriginPolicyTest` — `http://192.168.0.11:5173` 허용 / `http://evil.example.com` 거절 / `ftp://…` 거절 / `WORKLOG_ALLOWED_ORIGINS=203.0.113.0/24` 추가 허용
...
| `/settings/llm` · `/settings/notify` ADMIN 잠금 · 사원 명부 키 | ✅ 이미 됨 |
