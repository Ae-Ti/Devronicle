...
띄우고, 고치고, 확인하는 길. 프로젝트가 무엇인지는 [루트 README](../README.md) 에 있다.
자세한 요구사항은 [PRD](PRD_090910.md) 참고.

- 담당자 1 — 클라이언트: `frontend/`, `vscode-extension/`, 초안 CRUD API
- 담당자 2 — 연동: GitHub 수집, LLM 해석, 인증, Mattermost 알림
  - 1일차 계획: [DAY1_integration_plan.md](DAY1_integration_plan.md)
...
처음 클론했다면 **[SETUP.md](SETUP.md)** 를 본다. 아래는 요약이다.
...
비밀번호 변경을 먼저 시킨다.
사원번호 로그인은 사원 명부(외부 명부 API 또는 `DIRECTORY_FALLBACK_EMPLOYEES`)가 붙어야 된다.
...
이 저장소는 `main` 하나로 둔다. 만들던 동안에는 담당자별 트랙 브랜치(`track/client` ·
`track/integration`)를 두고 동기화 지점마다 통합 브랜치로 머지했다.
