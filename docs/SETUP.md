...
```bash
git clone https://github.com/Ae-Ti/Devronicle.git
cd Devronicle
...
| 사원번호 / 사원번호 | 사원 명부 API 나 폴백 명부가 붙은 뒤. 첫 로그인에 계정이 만들어진다 |
...
| **사원 명부** | `DIRECTORY_API_BASE_URL`·`DIRECTORY_API_KEY` (사내망 전용) | 사원번호 로그인이 안 된다. `DIRECTORY_FALLBACK_EMPLOYEES=9998:테스트사원` 으로 대신할 수 있다 |
...
1. 서버 PC 의 주소를 확인한다 — `ipconfig getifaddr en0` (예: `192.168.0.10`)
2. 다른 사람은 `http://192.168.0.10:5173` 을 연다. 자기 PC 에는 아무것도 안 띄워도 된다
