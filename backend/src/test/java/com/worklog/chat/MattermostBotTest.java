...
class MattermostBotTest {
    ...
    void answersOnceAndIgnoresSelf() {
        ...
        when(client.postsSince(eq(BASE), eq("tok"), eq("ch1"), anyLong()))
                .thenReturn(List.of(
                        post("p1", "bot-id", "김서준 오늘 업무일지", later), // 봇 계정 = 사람 계정인 경우
                        botPost("p2", "**김서준 · 업무 일지** …", later + 1),
                        post("p3", "someone", "점심 뭐 먹지", later + 2),
                        // 웹훅 알림 — "업무일지" 와 이름이 있어도 사람이 물은 것이 아니다
                        new MmPost("p4", "hook", "ch1", "김서준의 오늘(2026-09-11)의 업무일지가 요약되었습니다.", later + 3, "",
                                java.util.Map.of("from_webhook", "true", "override_username", "worklog")),
                        new MmPost("p5", "bot2", "ch1", "김서준 오늘 업무일지", later + 4, "",
                                java.util.Map.of("from_bot", "true"))))
                .thenReturn(List.of(post("p1", "bot-id", "김서준 오늘 업무일지", later))); // 경계의 글이 다시 온다
    ...
    void confirmsSummaryAfterNotification() {
        long later = System.currentTimeMillis() + 10_000;
        when(answers.answerDraft(7L)).thenReturn(Optional.of("**김서준 · 2026-09-11 업무 일지** …"));
        when(settings.effective()).thenReturn(
                new ChatBotSettingsService.Effective(BASE, "worklog-bot", "pw", true, null, "db", "ch1"));
        ...
        verify(client).createPost(BASE, "tok", "ch1", "**김서준 · 2026-09-11 업무 일지** …");
        ...
        when(client.postsSince(eq(BASE), eq("tok"), eq("ch1"), anyLong()))
                .thenReturn(List.of(post("p4", "someone", "김서준 오늘 업무일지", later + 4), post("p5", "someone", "넵", later + 5)));
        bot.poll();
