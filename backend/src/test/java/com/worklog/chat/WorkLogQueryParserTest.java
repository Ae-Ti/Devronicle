...
class WorkLogQueryParserTest {
    ...
    private static final List<String> NAMES = List.of("김서준", "배태일", "ungsikJo", "김민", "김민수");

    @Test
    @DisplayName("분명한 말(업무일지)과 느슨한 말(요약·뭐함)을 구분하고, 무관한 말은 거른다")
    void intent() {
        assertThat(WorkLogQueryParser.intentOf("김서준의 오늘 업무일지를 요약해서 보내줘")).isEqualTo(WorkLogQueryParser.Intent.STRONG);
        assertThat(WorkLogQueryParser.intentOf("김서준 업무요약")).isEqualTo(WorkLogQueryParser.Intent.WEAK);
        assertThat(WorkLogQueryParser.intentOf("배태일 어제 뭐했어?")).isEqualTo(WorkLogQueryParser.Intent.WEAK);
        assertThat(WorkLogQueryParser.intentOf("김서준 오늘 뭐함")).isEqualTo(WorkLogQueryParser.Intent.WEAK);
    ...
    void defaultsToToday() {
        assertThat(WorkLogQueryParser.dateIn("김서준의 오늘 업무일지", TODAY)).isEqualTo(TODAY);
        assertThat(WorkLogQueryParser.dateIn("김서준 업무일지", TODAY)).isEqualTo(TODAY);
    }
    ...
    void ranges() {
        assertThat(WorkLogQueryParser.rangeIn("김서준 9월 8일~9월 10일 업무일지 요약해서 줘", TODAY))
        ...
        assertThat(WorkLogQueryParser.rangeIn("9/8 - 9/10 배태일 일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 8), LocalDate.of(2026, 9, 10)));
        assertThat(WorkLogQueryParser.rangeIn("2026-09-08부터 2026-09-10까지 김서준 작업 내역", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 8), LocalDate.of(2026, 9, 10)));
        // 뒤쪽은 일만 적어도, 순서를 바꿔 적어도
        assertThat(WorkLogQueryParser.rangeIn("9월 8일 ~ 10일 김서준 일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 8), LocalDate.of(2026, 9, 10)));
        assertThat(WorkLogQueryParser.rangeIn("9/10~9/8 김서준 일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 8), LocalDate.of(2026, 9, 10)));
        // 상대 표현 (TODAY = 2026-09-10 목요일)
        assertThat(WorkLogQueryParser.rangeIn("김서준 이번 주 업무일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 7), TODAY));
        assertThat(WorkLogQueryParser.rangeIn("김서준 지난주 뭐함", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 8, 31), LocalDate.of(2026, 9, 6)));
        assertThat(WorkLogQueryParser.rangeIn("김서준 이번 달 업무일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 1), TODAY));
        assertThat(WorkLogQueryParser.rangeIn("김서준 최근 7일 업무일지", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 4), TODAY));
        assertThat(WorkLogQueryParser.rangeIn("김서준 3일간 작업 내역", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 8), TODAY));
        assertThat(WorkLogQueryParser.rangeIn("김서준 일주일 업무 정리", TODAY))
                .contains(new WorkLogQueryParser.DateRange(LocalDate.of(2026, 9, 4), TODAY));
    }
    ...
    void manyPeople() {
        assertThat(WorkLogQueryParser.peopleIn("배태일이랑 김서준 이번 주 업무일지", NAMES)).containsExactly("배태일", "김서준");
        assertThat(WorkLogQueryParser.peopleIn("김서준, 김민수, 배태일 어제 뭐함", NAMES)).containsExactly("김서준", "김민수", "배태일");
    ...
    void notARange() {
        assertThat(WorkLogQueryParser.rangeIn("김서준 9월 9일 업무일지", TODAY)).isEmpty();
        assertThat(WorkLogQueryParser.rangeIn("김서준 어제 업무일지", TODAY)).isEmpty();
        assertThat(WorkLogQueryParser.rangeIn("9/9~9/9 김서준 일지", TODAY)).isEmpty();
        assertThat(WorkLogQueryParser.rangeIn("김서준 1일간 일지", TODAY)).isEmpty();
    ...
    void person() {
        assertThat(WorkLogQueryParser.personIn("김서준의 오늘 업무일지", NAMES)).contains("김서준");
