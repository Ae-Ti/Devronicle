...
class WorkLogAnswerServiceTest {
    ...
    void setUp() {
        ...
        when(people.directory()).thenReturn(new PeopleDirectoryResponse(
                true, 29L,
                List.of(new PeopleDirectoryResponse.EmployeeRow(9999L, "김서준", null),
    ...
    void answersFromDraft() {
        ...
        String answer = service.answer("김서준의 오늘 업무일지를 요약해서 보내줘").orElseThrow();

        assertThat(answer).startsWith("**김서준 · " + KstDates.today() + " 업무 일지** _(확정본)_");
    ...
    void answersFromActivitiesWithoutSavingDraft() {
        Repo repo = new Repo();
        repo.setFullName("devronicle/worklog");
        ...
        String answer = service.answer("김서준 어제 업무일지").orElseThrow();
    ...
    void answersRange() {
        ...
        String answer = service.answer("김서준 9월 8일~9월 10일 업무일지 요약해서 줘").orElseThrow();

        assertThat(answer).startsWith("**김서준 · 2026-09-08 ~ 2026-09-10 업무 일지** _(3일 중 2일 기록)_");
    ...
    void answersSeveralPeopleInOrder() {
        ...
        String a = service.answer("배태일 김서준 오늘 업무일지").orElseThrow();
        assertThat(a.indexOf("**배태일** 님은")).isLessThan(a.indexOf("**김서준 · "));
        assertThat(a).contains("\n\n---\n\n").contains("로그인 고침");

        String b = service.answer("김서준이랑 배태일 오늘 업무일지").orElseThrow();
        assertThat(b.indexOf("**김서준 · ")).isLessThan(b.indexOf("**배태일** 님은"));

        // "김서준" 과 "UngsikJo" 는 같은 사람 — 한 번만
        String c = service.answer("김서준 UngsikJo 오늘 업무일지").orElseThrow();
    ...
    void rangeWithoutRecords() {
        ...
        String answer = service.answer("김서준 이번 주 업무일지").orElseThrow();
        assertThat(answer).contains("기록된 활동이 없습니다").contains(KstDates.today().toString());
    }

    @Test
    @DisplayName("'김서준 업무요약' 처럼 느슨한 말도 이름이 있으면 답한다")
    void weakIntentWithNameAnswers() {
        assertThat(service.answer("김서준 업무요약")).isPresent();
        assertThat(service.answer("김서준 오늘 뭐함")).isPresent();
    }
    ...
    void nothingRecorded() {
        String answer = service.answer("김서준 오늘 업무일지").orElseThrow();
    ...
    void unknownPerson() {
        ...
        assertThat(answer).contains("누구의 일지인지").contains("김서준");
