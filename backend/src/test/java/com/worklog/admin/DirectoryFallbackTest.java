 ...
 */
class DirectoryFallbackTest {

    private static DirectoryClient client(String baseUrl, String apiKey, String fallback) {
        DirectoryProperties p = new DirectoryProperties();
        p.setBaseUrl(baseUrl);
        p.setApiKey(apiKey);
        p.setFallbackEmployees(fallback);
        return new DirectoryClient(p);
    }
    ...
    void usesFallbackWhenNotConfigured() {
        var employees = client(null, null, "9999:김서준,9998:배태일").employees(0L);

        assertThat(employees).hasSize(2);
        assertThat(employees.get(0).empSeq()).isEqualTo(9999L);
        assertThat(employees.get(0).empNm()).isEqualTo("김서준");
    ...
    void tolerantOfSpaces() {
        assertThat(client(null, null, " 9999 : 김서준 , 9998 : 배태일 ").employees(0L)).hasSize(2);
    }

    @Test
    @DisplayName("형식이 틀린 항목은 건너뛰고 나머지를 살린다")
    void skipsMalformedEntries() {
        var employees = client(null, null, "9999:김서준,이건번호가없음,abc:xyz,9998:배태일").employees(0L);
    ...
    @Test
    @DisplayName("진짜 API 가 있어도 임시 항목은 뒤에 덧붙는다 — 사원 명부에 없는 사람으로 시험하려는 목적이다")
    void appendsToRealList() {
        // 사내망이 아니라 실제 목록은 비지만, 임시 항목은 그대로 붙어야 한다.
        var employees = client("http://127.0.0.1:1/none", "key", "9999:김서준,9998:배태일").employees(1L);

        assertThat(employees).hasSize(2);
        assertThat(employees).extracting(DirectoryClient.Employee::empSeq).containsExactly(9999L, 9998L);
    }
}
