...
class VscodeSessionServiceTest {
    ...
    private static final String REMOTE = "https://github.com/devronicle/attendance.git";
    ...
    void matchesRegisteredRepoOnly() {
        ...
        Repo repo = new Repo();
        repo.setId(1L);
        repo.setFullName("devronicle/attendance");
        when(repos.findByFullName("devronicle/attendance")).thenReturn(Optional.of(repo));
        assertThat(service.upsert(USER_ID, request(null)).getRepo()).isSameAs(repo);

        when(repos.findByFullName("devronicle/unknown")).thenReturn(Optional.empty());
        SessionRequest unknown = new SessionRequest(
                "https://github.com/devronicle/unknown.git", BRANCH, WORK_DATE,
