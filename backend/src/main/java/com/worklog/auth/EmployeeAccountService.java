...
import com.worklog.admin.DirectoryClient;
import com.worklog.admin.DirectoryProperties;
...
public class EmployeeAccountService {
    ...
    public EmployeeAccountService(
            UserRepository userRepository, DirectoryClient directoryClient, DirectoryProperties directoryProperties) {
    ...
    public Optional<User> provisionOnFirstLogin(String loginId, String password) {
        ...
        Long companySeq = directoryProperties.getCompanySeq();
        List<DirectoryClient.Employee> roster = directoryClient.employees(companySeq == null ? 0L : companySeq);
        if (roster.isEmpty()) {
            // 설정이 빠진 것과 "그런 사원이 없다" 는 다르다. 목록 자체가 비면 설정 문제다 —
            // 이 구분이 없으면 로그인이 안 되는 이유를 로그에서 찾을 수 없다.
            log.warn(
                    "사원 목록이 비어 있어 계정을 만들 수 없다. DIRECTORY_API_BASE_URL/DIRECTORY_API_KEY 또는"
                            + " DIRECTORY_FALLBACK_EMPLOYEES 를 확인한다 (companySeq={}).",
                    companySeq);
            return Optional.empty();
        }
        Optional<DirectoryClient.Employee> employee = roster.stream()
