...
public class DirectoryClient {
    ...
    public DirectoryClient(DirectoryProperties properties) {
     ...
     * 재직중 사원 목록.
     *
     * <p>{@code worklog.directory.fallback-employees} 에 적은 항목은 <b>실제 목록 뒤에 덧붙는다</b> —
     * 사내 API 가 없을 때는 그것만, 있을 때는 실제 사원 다음에 온다. 아직 사원 명부에 등록되지 않은
    ...
    /** {@code 9999:김서준,9998:배태일} 을 사원 목록으로 읽는다. */
    ...
    private <T> List<T> get(String path, String dataKey, Class<T> type) {
        if (!isConfigured()) {
            log.debug("사원 명부 API 설정이 없어 빈 목록을 돌려준다. ({})", path);
            return List.of();
        }
        try {
            Map<?, ?> body = restClient
            ...
            if (body == null || !(body.get("data") instanceof Map<?, ?> data)) {
                log.warn("사원 명부 응답에 data 가 없다: {}", path);
                return List.of();
            }
            if (!(data.get(dataKey) instanceof List<?> items)) {
                log.warn("사원 명부 응답에 data.{} 가 없다: {}", dataKey, path);
                return List.of();
            }
            ...
            log.warn("사원 명부 API 호출 실패 ({}): {}", path, e.getMessage());
            return List.of();
        }
    }
