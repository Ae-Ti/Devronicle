            ...
            <CardContent className="p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground">
                  ...
                  {!data.directoryConfigured && data.employees.length > 0 && ' · 임시 목록'}
                </span>
              </div>
              ...
              {data.employees.length === 0 ? (
                <p className="rounded border border-dashed p-4 text-[13px] text-muted-foreground">
                  ...
                  <code className="rounded bg-muted px-1 text-xs">DIRECTORY_API_BASE_URL</code>,{' '}
                  <code className="rounded bg-muted px-1 text-xs">DIRECTORY_API_KEY</code>,{' '}
                  <code className="rounded bg-muted px-1 text-xs">DIRECTORY_COMPANY_SEQ</code> 를 채우면
                  이 표가 채워집니다.
                </p>
              ) : (
                <>
                  {!data.directoryConfigured && (
                    <p className="mb-3 rounded border border-dashed p-2.5 text-[13px] text-muted-foreground">
                      사내 회원 조회 API가 아직 설정되지 않아{' '}
                      <code className="rounded bg-muted px-1 text-xs">DIRECTORY_FALLBACK_EMPLOYEES</code>{' '}
