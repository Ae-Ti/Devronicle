...
async function askServerUrl(step?: string): Promise<string | undefined> {
  ...
  const answer = await vscode.window.showInputBox({
    ...
    prompt: '대시보드를 여는 주소가 아니라 백엔드 주소입니다. 예) http://192.168.0.10:8080',
    value: current,
    ignoreFocusOut: true, // 서버를 띄운 사람에게 주소를 물어보는 동안 닫히면 안 된다
    validateInput: (v) =>
      normalizeServerUrl(v) ? undefined : '주소:포트 형태로 적어 주세요. 예) http://192.168.0.10:8080',
  })
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
   ...
   * vscode://devronicle.worklog-drafter/connect?server=…&key=…
   *
