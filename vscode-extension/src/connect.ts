...
/** 대시보드 링크가 들어오는 자리. `vscode://devronicle.worklog-drafter/connect` 의 뒷길이다. */
export const CONNECT_URI_PATH = '/connect'

/**
 * 사람이 적은 서버 주소를 쓸 수 있는 형태로 다듬는다.
 *
 * <p>`192.168.0.10:8080` 처럼 스킴 없이 적거나, 주소창에서 복사해 끝 슬래시나 `/api` 를
 ...
 * <ul>
 *   <li>{@code wlc1_...} — 연결 코드
 *   <li>{@code vscode://devronicle.worklog-drafter/connect?server=...&key=...} — 링크
...
export function decodeConnection(raw: string): Connection | undefined {
  ...
  // 이미 풀어서 건네는 쿼리(`server=http://192.168.0.10:8080&key=…`)가 링크로 잘못 잡혀
