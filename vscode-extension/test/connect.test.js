...
test('대시보드 링크를 통째로 붙여 넣어도 읽는다', () => {
  const link = `vscode://devronicle.worklog-drafter/connect?server=${encodeURIComponent(SERVER)}&key=${KEY}`
...
test('코드를 링크에 실어 보내도 읽는다', () => {
  const code = encodeConnection({ serverUrl: SERVER, apiKey: KEY })
  assert.deepEqual(
    decodeConnection(`vscode://devronicle.worklog-drafter/connect?code=${code}`),
    { serverUrl: SERVER, apiKey: KEY },
  )
})

test('주소는 다듬어 들어간다 — 끝 슬래시와 /api 는 Uploader 가 붙인다', () => {
  const c = decodeConnection(`server=${encodeURIComponent('192.168.0.10:8080/api/')}&key=${KEY}`)
...
test('반쪽짜리나 망가진 것은 받지 않는다', () => {
  ...
  assert.equal(
    decodeConnection(`server=${encodeURIComponent('ftp://192.168.0.10')}&key=${KEY}`),
    undefined,
    'http 가 아니다',
  )
